  ...
  async getCreationIds(): Promise<SlideInfo[]> {
    ...
    for (const slideRel of relationships) {
      try {
        ...
        const slidePath = 'ppt/' + slideRel.file;
        if (!FileHelper.fileExistsInArchive(archive, slidePath)) {
          // Skip files that don't exist in the archive
          console.log(`Skipping non-existent slide file: ${slidePath}`);
          continue;
        }

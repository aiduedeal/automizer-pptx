...
export default class ModifyPresentationHelper {
   ...
   */
  static getSlidesCollection = (xml: XmlDocument | XmlElement) => {
    return xml.getElementsByTagName('p:sldId');
  };
  static getSlideMastersCollection = (xml: XmlDocument | XmlElement) => {
   ...
   */
  static sortSlides = (order: number[]) => (xml: XmlDocument | XmlElement) => {
   ...
   * First slide starts by 1.
   * @param numbers Array of slide numbers to remove, starting from 1
   */
  static removeSlides = (numbers: number[]) => (xml: XmlDocument | XmlElement) => {
    const slides = ModifyPresentationHelper.getSlidesCollection(xml);
    
    // Convert 1-based slide numbers to 0-based indices
    const adjustedNumbers = numbers.map(num => num - 1);
    
    // Remove slide references from presentation.xml
    for (let i = 0; i < slides.length; i++) {
      if (adjustedNumbers.includes(i)) {
  ...
  static async removeUnusedFiles(
    ...
    // Get slide IDs in the presentation (those that weren't removed)
    const activeSlideIds = new Set<string>();
    const slides = ModifyPresentationHelper.getSlidesCollection(xml);
    for (let i = 0; i < slides.length; i++) {
      const slideNum = i + 1;
      activeSlideIds.add(`slide${slideNum}.xml`);
      activeSlideIds.add(`slide${slideNum}.xml.rels`);
    }
    ...
    for (const dir in Tracker.files) {
      ...
      await FileHelper.removeFromDirectory(archive, dir, (file) => {
        // For slides directories, only keep files for active slides
        if (dir === 'ppt/slides' || dir === 'ppt/slides/_rels') {
          return !activeSlideIds.has(file.relativePath);
        }
        // For other directories, use the default tracking

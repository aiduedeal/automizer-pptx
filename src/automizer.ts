...
export default class Automizer implements IPresentationProps {
  ...
  /**
   * Directly modify presentation to physically remove slide2.xml files
   * Used to complete slide removal
   */
  private async cleanupRemovedSlides(): Promise<void> {
    // Get remaining slide numbers from presentation.xml
    const presXml = await XmlHelper.getXmlFromArchive(
      this.rootTemplate.archive,
      'ppt/presentation.xml'
    );
    
    const remainingSlideIds = new Set<number>();
    const slides = presXml.getElementsByTagName('p:sldId');
    for (let i = 0; i < slides.length; i++) {
      remainingSlideIds.add(i + 1); // 1-based slide numbers
    }
    
    // Check all slide files and remove those not in remaining slides
    try {
      const slideFiles = await this.rootTemplate.archive.folder('ppt/slides');
      for (const file of slideFiles) {
        const match = file.name.match(/slide(\d+)\.xml/);
        if (match) {
          const slideNum = parseInt(match[1], 10);
          if (!remainingSlideIds.has(slideNum)) {
            // Remove slide file and its relationships
            await this.rootTemplate.archive.remove(file.name);
            const relsFile = `ppt/slides/_rels/${file.relativePath}.rels`;
            if (FileHelper.fileExistsInArchive(this.rootTemplate.archive, relsFile)) {
              await this.rootTemplate.archive.remove(relsFile);
            }
          }
        }
      }
    } catch (error) {
      console.error('Error cleaning up slide files:', error);
    }
  }
  ...
  public async write(location: string): Promise<AutomizerSummary> {
    ...
    // Added cleanup step for removed slides
    await this.cleanupRemovedSlides();
